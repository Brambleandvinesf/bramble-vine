...
export function StateDebrief({
  ...
  const [addedDone, setAddedDone] = useState<
    Array<{ id: string; action: string; client: string }>
  >([]);
  /* (8/5) The completed/follow-up toggle is GONE. This screen creates
     done-this-visit work only; follow-ups are created on the Future Projects
  ...
  const saveAddedProject = useCallback(async () => {
    ...
    try {
      const { projectId, crossed } = await addCompletedProject({
        ...
        type: addDone.type,
        notes: addDone.notes,
        /* (8/5) Garden and Category were being collected by the shared
           NewProjectForm and then dropped here — the payload never carried them,
           so picking either on this screen silently did nothing. Both are
           accepted by createProject and always have been. */
        garden: addDone.garden,
        category: addDone.category,
      });
      setAddedDone((cur) => [
        ...cur,
        { id: projectId, action: addDone.action.trim(), client: target },
      ]);
          ...
          <div>
            ...
            <div style={{ marginTop: 14, borderTop: `1px solid ${LINE}`, paddingTop: 12 }}>
              {addedDone.map((a) => (
                ...
                >
                  ✓ {a.action}{" "}
                  <span style={{ color: MUTED }}>· logged as {a.id}</span>
                </div>
              ))}
              {addErr && (
              ...
              {addDone ? (
                <>
                  <NewProjectForm
                    ...
                    cameraDisabled={addBusy || isPreview || !clientMatch}
                ...
                <button
                  onClick={() => {
                    setAddDone(newProjectRow("SPECIAL"));
                    setAddErr(null);
                  }}
                  disabled={isPreview}
