...
earlier batches as an artefact of that split, not as disagreement.

**1. FINANCIAL CONNECTIONS — INVESTIGATED AND REJECTED for real-time purchase detection.**
It is **daily-batch only**, confirmed from Stripe's own docs. **Mercury is the viable
real-time option instead.** ⚠ Mercury account setup has NOT happened and remains a future
step — nothing is built against it.

**2. WHAT STRIPE IS NOT USED FOR — three explicit rejections, each with its reason:**
· 🚫 **NO CARD SURCHARGING.** Preview-gated, needs a third-party vendor, and carries real
  legal exposure — too much compliance overhead for this business's scale.
· 🚫 **NO STRIPE TAX.** QuickBooks already calculates the correct tax-inclusive total, and
  **Stripe must never recalculate it.** Two tax engines on one invoice is a wrong number
  waiting to happen.
· 🚫 **NO CARD ACCEPTANCE.** Credit cards stay **entirely with SwipeSimple, unchanged.**

**3. WHAT STRIPE IS USED FOR: a plain, surcharge-free ACH-ONLY Payment Link per invoice,
embedded inside a CUSTOM-BUILT invoice PDF.** New `invoiceHtml_` / `invoicePayPdf_` pair,
**a SIBLING of `receiptHtml_`/`saveReceipt`, NOT a modification of the receipt path.**
⚠ **THIS REPLACES the Drive-token-page-linking-to-a-QBO-PDF approach built in CC-95/CC-97.**
The new PDF, generated by B&V's own code, IS the document shared with clients, with the
Stripe link baked in. The `?inv=` token page and `invoicePdfUrlFor_` are superseded — do
not extend them without deciding which approach is current.

**4. ACH DIRECT DEBIT (`us_bank_account`) CHOSEN OVER "INSTANT BANK PAYMENTS", on cost.**
Instant Bank Payments would cost **2.6% + 30¢** — nearly as much as a credit card —
against ACH's **0.8% capped at $5**. Confirmed from Stripe's pricing docs. Choosing the
faster option would have defeated the entire purpose of avoiding cards.

**5. ONE-SHOT LINKS: `restrictions[completed_sessions][limit] = 1`.** Deliberately chosen
over allowing retries, and the reasoning is the important part: **a successful-but-reused
link risks a real double payment, which is worse than a failed payment needing a manually
re-minted link.** ⚠ And QBO reconciliation stays **MANUAL** for now (Gap 1, Option 1) — no
webhook auto-marking invoices paid.

**6. ⚠⚠ TECHNICAL STATE — THE PI AND THE WEB EDITOR HAD DIVERGED, AND A DEPLOY WOULD HAVE
DESTROYED THE WORK.** `invoiceHtml_`, `invoicePayPdf_` and `stripePaymentLinkFor_` were
pasted DIRECTLY into the live Apps Script web editor and tested there (a real test-mode
Payment Link was created and confirmed working). **They did not exist in the Pi's Code.js.**
✅ **RECONCILED IN CC-112** by `clasp pull` (the safe direction), after backing the Pi copy
up as `Code.js.bak.PRE-CC112-RECONCILE.*`. All three are now present at lines ~20986,
~21044 and ~21058, and `node --check` passes.
🚫 **THE STANDING HAZARD, NOW PROVEN REAL: `bv-deploy.sh` RUNS `clasp push` FIRST.** Any
...
**7. 🚫 THE `STRIPE_RK` → `STRIPE_SECRET_KEY` RENAME WAS NEVER DONE — AND THAT IS THE WHOLE
"MYSTERY".** The live code still reads **`STRIPE_RK`** (lines 21056, 21059, 21060):
      const rk = PropertiesService.getScriptProperties().getProperty('STRIPE_RK');
      if (!rk) throw new Error('STRIPE_RK not set');
**So every attempt to set `STRIPE_SECRET_KEY` was setting a property nothing reads.** That
fully explains "multiple attempts have not been read correctly by the code" — the value was
fine, the name was wrong.
⚠ **AND `cc110PropsProbe` DOES NOT EXIST IN THE CODE EITHER**, which is why it could never
be run. Both halves of that investigation were chasing things that were not there.
⚠ THE RENAME IS STILL WORTH DOING, for the reason originally given: `SECRET_KEY_RE` in
`propSnapshot_` auto-redacts anything matching `*SECRET_KEY*`, and `STRIPE_RK` is **not**
redacted — so the live Stripe key is currently exposed in property snapshots.
